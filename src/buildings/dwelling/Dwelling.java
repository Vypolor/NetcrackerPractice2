...
public class Dwelling implements Building, Serializable, Cloneable, Iterable<Floor> {
    ...
    public void changeFloor(int num, Floor iFloor) throws FloorIndexOutOfBoundsException {
        if(num < 0 || num >= getGenSpacesNum()) throw new FloorIndexOutOfBoundsException(num,getGenSpacesNum());
        /*Floor floor = new DwellingFloor(iFloor.getSpacesNum());
        for(int i = 0; i < floor.getSpacesNum(); ++i){
            floor.changeSpace(i, iFloor.getSpace(i));
        }*/
    ...
    public String toString(){
        String s = new String("Dwelling (" + this.getFloorsNum());
    ...
    public boolean equals(Object object){
        boolean flag = false;
        if((object instanceof Dwelling)&&(this.getFloorsNum() == ((Dwelling) object).getFloorsNum())&&
                (this.getGenSpacesNum() == ((Dwelling) object).getGenSpacesNum())){
    ...
    public void print(){
        ...
        System.out.println("Number of Floors: " + _floorsNum);
        System.out.println();
        int flat_number = 0;
        for(int i = 0; i < _floorsNum; ++i){

            System.out.println("\n\t=== Floor #" + i + " ===\n");
            for(int j = flat_number; j < flat_number + _arr[i].getSpacesNum(); ++j){
                System.out.print("== '" + j + "' ");
                _arr[i].getSpacesArr()[j - flat_number].print();
            }
            flat_number +=_arr[i].getSpacesNum();
